...
url = "https://www.variety.com/sitemap_index.xml"
website = Website(url)
...
with open(f'kiki_web_scrap/newspapers/variety/{round(now)}_links.txt', 'w',  encoding="utf-8") as a:
    a.write("")
...
for _ in list_of_all_sitemaps[:]:
    ...
    pattern2 = "https:\/\/variety\.com\/.*?(?=\s|<|$)"
    list_of_links = re.findall(pattern2, html)
    if len(list_of_links) > 0:
        total_len += len(list_of_links)
        print(len(list_of_links), "", total_len)
    with open(f'kiki_web_scrap/newspapers/variety/{round(now)}_links.txt', 'a',  encoding="utf-8") as a:
